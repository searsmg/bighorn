...
# read in stage data
stage <- read_csv(here('raw_data', 'bighorn_stage_composite.csv')) %>%
...
stage_obs <- read_csv(here('raw_data', 'bighorn_stageplate.csv')) %>%
  mutate(datetime = mdy_hm(datetime)) %>%
  rename(dt_use = datetime)
...
stage_corr <- stage %>%
  ...
  mutate(., stage_corr_cm = ifelse(dt_use > ymd_hms('2021-05-07 12:00:00'),
    ...
    mutate(., stage_corr_cm = ifelse(dt_use > ymd_hm('2022-05-03 13:15'),
                         stage_corr_cm + 2, stage_corr_cm)) %>%
      mutate(., stage_corr_cm = ifelse(dt_use > ymd_hm('2022-05-19 14:15'),
                         stage_corr_cm + 6, stage_corr_cm)) %>%
  mutate(., stage_corr_cm = ifelse(stage_corr_cm < 4, NA, stage_corr_cm)) %>%
  mutate(., stage_corr_cm = ifelse(dt_use > ymd_hm('2023-04-18 10:00'),
                         stage_corr_cm - 9.5, stage_corr_cm)) #%>%
  # mutate(., stage_corr_cm = ifelse(dt_use > ymd_hm('2023-05-24 10:00'),
  #                        stage_corr_cm - 4.5, stage_corr_cm)) %>%
  # mutate(., stage_corr_cm = ifelse(dt_use > ymd_hm('2023-05-31 11:30'),
  #                        stage_corr_cm - 3, stage_corr_cm)) %>%
  # mutate(., stage_corr_cm = ifelse(dt_use > ymd_hm('2023-06-06 10:00'),
  #                        stage_corr_cm - 2, stage_corr_cm)) %>%
  # mutate(., stage_corr_cm = ifelse(dt_use > ymd_hm('2023-06-16 09:00'),
  #                        stage_corr_cm - 7, stage_corr_cm))


stage_plot <- ggplot(stage_corr, aes(dt_use, stage_corr_cm)) +
  geom_line() +
  geom_point(data = stage_obs, aes(dt_use, manual_stage_cm),
             color = 'blue')
